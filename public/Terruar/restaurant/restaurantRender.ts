import {
  getCategoriesWithProducts,
  type Product,
  type Category,
} from './restaurantFetch';
import { PopUp } from '../_new/ui/popUp/popUp';
import './restaurantStyles.css';

const WAIcon =
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" color="currentColor"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12c0 1.379.28 2.693.784 3.888c.279.66.418.99.436 1.24c.017.25-.057.524-.204 1.073L2 22l3.799-1.016c.549-.147.823-.22 1.073-.204c.25.018.58.157 1.24.436A10 10 0 0 0 12 22"/><path d="m8.588 12.377l.871-1.081c.367-.456.82-.88.857-1.488c.008-.153-.1-.841-.315-2.218C9.916 7.049 9.41 7 8.973 7c-.57 0-.855 0-1.138.13c-.358.163-.725.622-.806 1.007c-.064.305-.016.515.079.935c.402 1.783 1.347 3.544 2.811 5.009c1.465 1.464 3.226 2.409 5.01 2.811c.42.095.629.143.934.079c.385-.08.844-.448 1.008-.806c.129-.283.129-.568.129-1.138c0-.438-.049-.943-.59-1.028c-1.377-.216-2.065-.323-2.218-.315c-.607.036-1.032.49-1.488.857l-1.081.87"/></g></svg>';

//
function getProductElement(prod: Product, index: number): HTMLDivElement {
  const proDiv = document.createElement('div');
  proDiv.className = 'product';

  proDiv.style.animationDelay = `${index * 0.05}s`;

  proDiv.innerHTML = /* html */ `
  <div class="product-header">
    <img src="${prod.photoList[0]}"
        loading="lazy" referrerpolicy="no-referrer"
        alt="${prod.title}"
        crossorigin="anonymous" />
    </div>
    <div class="product-info">
      <p class="product-title"> ${prod.title} </p>
      <p class="product-descr"> ${prod.descr} </p>
    </div>
    <div class="product-footer">
      <a href="https://wa.me/79670655655?text=Здравствуйте! Я хочу заказать ${prod.category}: ${prod.title}" target="_blank" rel="noopener noreferrer"> 
        <span>Заказать </span> ${WAIcon}</a>
        <p class="product-price"> ${prod.price} ₽ </p>
        </div>
        `;

  return proDiv;
}
async function insertProducts(): Promise<HTMLDivElement> {
  const catalogie = document.createElement('div');
  catalogie.classList.add('catalogue');

  // const categories = await getCategoriesWithProducts();
  const categories = _exampleProducts;

  categories.forEach((category) => {
    if (category.products.length === 0) return;
    const categoryDiv = document.createElement('div');
    categoryDiv.classList.add('category');
    categoryDiv.id = `${category.title}`;

    const categoryTitle = document.createElement('div');
    categoryTitle.textContent = category.title;
    categoryTitle.classList.add('cat-title');

    categoryDiv.appendChild(categoryTitle);

    category.products.forEach((prod, index) => {
      const productElement = getProductElement(prod, index);
      categoryDiv.appendChild(productElement);
    });

    catalogie.appendChild(categoryDiv);
  });

  return catalogie;
}

//
async function initCatalogue() {
  const catalogue = await insertProducts();
  const cataloguePopUp = new PopUp('Готовая еда в глэмпинге', catalogue);

  const searchParams = new URLSearchParams(location.search);
  const isUrlHasAction = searchParams.has('open-rest');

  const restButtons = document.querySelectorAll(
    '[data-action="open-restaurant-menu"]'
  );
  if (restButtons.length) {
    restButtons.forEach((button) => {
      button.addEventListener('click', () => cataloguePopUp.open());
    });
  }

  if (isUrlHasAction) {
    cataloguePopUp.open();
  }
}
window.addEventListener('load', initCatalogue);

const _exampleProducts: Category[] = [
  {
    uid: 465704772022,
    title: 'Завтраки',
    sort: 100,
    products: [
      {
        title: 'Блины с сыром и ветчиной',
        descr:
          'Молоко, мука пшеничная, сыр полутвердый, соль, ветчина, яйцепродукты, масло подсолнечное, сахар, зелень, сливки, соль.',
        price: 300,
        photoList: [
          'https://thumb.tildacdn.com/stor3834-3563-4732-a536-663537343462/-/format/webp/72297990.png.webp',
        ],
        category: 'Завтраки',
        weight: 150,
        specs: {
          Калории: '240',
          Жиры: '11',
          Белки: '12',
          Углеводы: '22',
        },
      },
      {
        title: 'Блины с яблоком и корицей',
        descr:
          'Мука, молоко, яичный меланж, сахар, яблоко, масло подсолнечное, масло сливочное',
        price: 300,
        photoList: [
          'https://thumb.tildacdn.com/stor3161-6639-4739-b164-323563633738/-/format/webp/52923288.png.webp',
        ],
        category: 'Завтраки',
        weight: 250,
        specs: {
          Калории: '260',
          Жиры: '16',
          Белки: '8',
          Углеводы: '21',
        },
      },
      {
        title: 'Каша рисовая с яблоком',
        descr:
          'Молоко, крупа рисовая, яблоки сушеные, сахар, маслосливочное, соль, крахмал картофельный.',
        price: 300,
        photoList: [
          'https://thumb.tildacdn.com/stor6231-3966-4535-b063-383536356134/-/format/webp/19981322.png.webp',
        ],
        category: 'Завтраки',
        weight: 250,
        specs: {
          Калории: '110',
          Жиры: '3',
          Белки: '1,5',
          Углеводы: '18',
        },
      },
      {
        title: 'Скрембл с цыпленком и грибами',
        descr:
          'Меланж яичный, яичный желток, мясо муриное, шампиньоны, молоко, сливки питьевые, масло сливочное, сыр моцарелла, специи',
        price: 300,
        photoList: [
          'https://thumb.tildacdn.com/stor3430-3865-4262-b636-383233303437/-/format/webp/62995171.png.webp',
        ],
        category: 'Завтраки',
        weight: 150,
        specs: {
          Калории: '220',
          Белки: '12',
          Жиры: '17',
          Углеводы: '2',
        },
      },
      {
        title: 'Сырники с малиновым вареньем',
        descr:
          'Творог, яичный меланж, мука, крупа манная, сахар, соль, масло подсолнечное, соус ягодный (малина, сахар, натуральный загуститель)',
        price: 300,
        photoList: [
          'https://thumb.tildacdn.com/stor3063-6430-4965-a130-636565313366/-/format/webp/95594882.png.webp',
        ],
        category: 'Завтраки',
        weight: 180,
        specs: {
          Калории: '280',
          Жиры: '17',
          Белки: '12',
          Углеводы: '2',
        },
      },
    ],
  },
  {
    uid: 382030958842,
    title: 'Горячее',
    sort: 200,
    products: [
      {
        title: 'Куриные котлеты с пюре и сырным соусом',
        descr:
          'Картофельное пюре, молоко, соль. Мясо птицы, лук репчатый, масло подсолнечное, сухари, яйца, специи, соус (сыр, сливки, мука)',
        price: 450,
        photoList: [
          'https://thumb.tildacdn.com/stor3938-6232-4537-b235-616137626562/-/format/webp/20318009.png.webp',
        ],
        category: 'Горячее',
        weight: 260,
        specs: {
          Калории: '190',
          Жиры: '12',
          Белки: '10',
          Углеводы: '11',
        },
      },
      {
        title: 'Кюфта из баранины с картофелем',
        descr:
          'Кюфта (говядина, лук, баранина, сухари панировочные, меланж яичный, орехгрецкий, масло подсолнечное, соль, чеснок, перец, орех мускатный), картофель, томаты в заливке, вода, перец сладкий, лук, крахмал картофельный, сахар, смесь специй "Аджика", сельдерей',
        price: 600,
        photoList: [
          'https://thumb.tildacdn.com/stor3466-6163-4265-a136-646162306666/-/format/webp/54067032.png.webp',
        ],
        category: 'Горячее',
        weight: 271,
        specs: {
          Калории: '247',
          Жиры: '15',
          Белки: '20',
          Углеводы: '12',
        },
      },
      {
        title: 'Паста с морепродуктами',
        descr:
          'Макароны, кальмары, томаты, креветки, сливки питьевые, масло подсолнечное, сыр полутвердый, мука пшеничная, масло сливочное, зелень, специи.',
        price: 600,
        photoList: [
          'https://thumb.tildacdn.com/stor3734-6238-4762-b931-663337646336/-/format/webp/57422108.png.webp',
        ],
        category: 'Горячее',
        weight: 260,
        specs: {
          Калории: '130',
          Белки: '9',
          Жиры: '3',
          Углеводы: '16',
        },
      },
      {
        title: 'Спагетти Карбонара с копченым цыпленком',
        descr:
          'Молоко, макароны, мясо куриное, сыр пармезан, лук репчатый, сыр моцарелла, яйца, масло подсолнечное, мука пшеничная, масло сливочное, соль',
        price: 450,
        photoList: [
          'https://thumb.tildacdn.com/stor3838-3035-4039-b163-343331306134/-/format/webp/85053857.png.webp',
        ],
        category: 'Горячее',
        weight: 230,
        specs: {
          Калории: '210',
          Жиры: '11',
          Белки: '9',
          Углеводы: '16',
        },
      },
      {
        title: 'Фрикадельки с рисом и соусом Черный перец',
        descr: '',
        price: 400,
        photoList: [
          'https://thumb.tildacdn.com/stor3134-6238-4834-a535-376636393961/-/format/webp/14528953.png.webp',
        ],
        category: 'Горячее',
        specs: {},
      },
    ],
  },
  {
    uid: 159820108342,
    title: 'Супы',
    sort: 300,
    products: [
      {
        title: 'Куриный суп-лапша',
        descr:
          'Мясо птицы, макароны, морковь, лук, масло подсолнечное, зелень, специи',
        price: 300,
        photoList: [
          'https://thumb.tildacdn.com/stor3563-3363-4965-a665-613435353134/-/format/webp/45233046.png.webp',
        ],
        category: 'Супы',
        weight: 300,
        specs: {
          Калории: '69',
          Жиры: '2,5',
          Белки: '2,5',
          Углеводы: '6,5',
        },
      },
      {
        title: 'Борщ с телятиной без сметаны',
        descr:
          'Говядина, капуста, картофель, лук, свекла, морковь, томатная паста, перец сладкий масло подсолнечное, специи',
        price: 350,
        photoList: [
          'https://thumb.tildacdn.com/stor3132-3261-4862-a534-353938376637/-/format/webp/18894473.png.webp',
        ],
        category: 'Супы',
        weight: 300,
        specs: {
          Калории: '60',
          Жиры: '3',
          Белки: '3',
          Углеводы: '5,5',
        },
      },
      {
        title: 'Уха Скандинавская',
        descr:
          'Рыба минтай, молоко, рыба лососевых пород, картофель, томаты , лук, сливки перец сладкий, масло подсолнечное, масло сливочное, мука, морковь, зелень.',
        price: 450,
        photoList: [
          'https://thumb.tildacdn.com/stor6331-6166-4137-b832-666464366337/-/format/webp/73599619.png.webp',
        ],
        category: 'Супы',
        weight: 300,
        specs: {
          Калории: '145',
          Жиры: '10',
          Белки: '8',
          Углеводы: '5,5',
        },
      },
    ],
  },
  {
    uid: 809493334032,
    title: 'Пицца',
    sort: 400,
    products: [
      {
        title: 'Маргарита',
        descr:
          'Легендарный вкус Италии. Простое и гениальное сочетание спелых томатов, нежной моцареллы и свежего базилика.',
        price: 650,
        photoList: [
          'https://thumb.tildacdn.com/stor6133-6437-4233-b033-313163326236/-/format/webp/27765905.png.webp',
        ],
        category: 'Пицца',
        specs: {},
      },
      {
        title: 'Пепперони',
        descr:
          'Нестареющая классика. Яркий и в меру острый вкус колбасок пепперони с большим количеством сыра на томатном соусе.',
        price: 700,
        photoList: [
          'https://thumb.tildacdn.com/stor3031-6531-4864-b633-366431353631/-/format/webp/45963419.png.webp',
        ],
        category: 'Пицца',
        specs: {},
      },
      {
        title: 'Груша и горгонзола',
        descr:
          'Изысканное сочетание для гурманов. Нежная сладость груши идеально дополняет пикантный вкус сыра горгонзола.',
        price: 700,
        photoList: [
          'https://thumb.tildacdn.com/stor3433-3962-4435-b831-633465643261/-/format/webp/45714064.png.webp',
        ],
        category: 'Пицца',
        specs: {},
      },
      {
        title: 'Мясная',
        descr:
          'Настоящее мясное изобилие для самых голодных. Аппетитное сочетание нескольких видов мяса под щедрым слоем тягучей моцареллы.',
        price: 850,
        photoList: [
          'https://thumb.tildacdn.com/stor6136-6636-4233-b935-303763653065/-/format/webp/57659272.png.webp',
        ],
        category: 'Пицца',
        specs: {},
      },
    ],
  },
  {
    uid: 219944136122,
    title: 'Десерты',
    sort: 500,
    products: [
      {
        title: 'Панкейк с шоколадно-банановым соусом',
        descr:
          'Молоко, мука, масло растительное, бананы, шоколад темный, сахар, яичный меланж, соль, пищевая сода.',
        price: 300,
        photoList: [
          'https://thumb.tildacdn.com/stor6538-6533-4065-b238-633832313530/-/format/webp/43544010.png.webp',
        ],
        category: 'Десерты',
        weight: 150,
        specs: {
          Калории: '210',
          Белки: '7',
          Жиры: '12',
          Углеводы: '18',
        },
      },
      {
        title: 'Шоколадный брауни',
        descr:
          'Сахар, масло сливочное, яичный меланж, сливки, какао, шоколад, мука миндальная, сироп, соль, соус маракуйя.',
        price: 400,
        photoList: [
          'https://thumb.tildacdn.com/stor6561-3465-4135-b738-626131643734/-/format/webp/57566551.png.webp',
        ],
        category: 'Десерты',
        weight: 120,
        specs: {
          Калории: '510',
          Белки: '8,5',
          Жиры: '7',
          Углеводы: '18,5',
        },
      },
    ],
  },
];
